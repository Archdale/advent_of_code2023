...
class OasisSensor {
  public:
    struct Reading {
        std::vector<std::deque<int32_t>> dataset{};
        
        bool is_complete() {
            std::deque<int32_t> empty(dataset.back().size());
    ...
    uint32_t process() {
        ...
        // Extrapolate
        for (auto& reading : readings_) {
            reading.dataset.back().push_front(0);
            auto previous = reading.dataset.rbegin();
            for (auto data = reading.dataset.rbegin() + 1; data != reading.dataset.rend(); data++) {
                data->push_front(data->front() - previous->front());
                previous = data;
            }
            retval += reading.dataset.front().front();
        }
